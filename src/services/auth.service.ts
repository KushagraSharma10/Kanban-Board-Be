...
import { findUserByEmail, createUser, findUserById, setRefreshTokenForUser, verifyUserRefreshToken, clearRefreshTokenForUser } from "../dao/user.dao.js";
...
const computeRefreshExpiryDate = (): Date => {
  const days = Number(String(process.env.REFRESH_TOKEN_EXPIRES || "7d").replace("d", "")) || 7;
  const date = new Date();
...
export const registerUserCore = async(input: RegisterInput) => {
  ...
  await setRefreshTokenForUser(created._id as Types.ObjectId, refreshToken, refreshExpiresAt);

  return {
    user: { id: String(created._id), fullName: created.fullName, email: created.email, role: created.role },
    accessToken,
    refreshToken,
    refreshExpiresAt,
  };
...
export const loginUserCore = async(input: LoginInput) => {
 ...
 const refreshToken = signRefreshToken(String(user._id));
  const refreshExpiresAt = computeRefreshExpiryDate();
  await setRefreshTokenForUser(user._id as Types.ObjectId, refreshToken, refreshExpiresAt);

  return {
    user: { id: String(user._id), fullName: user.fullName, email: user.email, role: user.role },
    accessToken,
    refreshToken,
    refreshExpiresAt,
  };
...
export const refreshTokensCore = async (refreshTokenFromCookie: string) => {
  ...
  const refreshExpiresAt = computeRefreshExpiryDate();
  await setRefreshTokenForUser(user._id as Types.ObjectId, newRefreshToken, refreshExpiresAt);
